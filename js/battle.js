...
// and pushes the same event types the animator understands. No trait hooks and
// no auto-send-next (the interactive controller handles replacement). Mirrors
// the non-trait damage/item subset of runBattle.
function resolveOneAttack(attacker, aIdx, side, target, tIdx, tSide, move, overtimeMult, events) {
  const attackerItems = attacker.heldItem ? [attacker.heldItem] : [];
  const defenderItems = target.heldItem ? [target.heldItem] : [];
  const aName = attacker.nickname || attacker.name;
  const tName = target.nickname || target.name;

  if (move.noDamage) {
    events.push({ type: 'attack', side, attackerIdx: aIdx, attackerName: aName,
      targetSide: tSide, targetIdx: tIdx, targetName: tName,
      moveName: move.name, moveType: move.type, damage: 0, typeEff: 1, crit: false,
      isSpecial: false, attackerHpAfter: attacker.currentHp, targetHpAfter: target.currentHp });
    return;
  }

  // If the chosen move has no effect on the target, fall back to Struggle.
  let m = move;
  if (getTypeEffectiveness(m.type, target.types || ['Normal']) === 0) {
    m = { name: 'Struggle', power: 50, type: 'Normal', isSpecial: false, typeless: true };
  }

  const { damage: rawDamage, typeEff, moveType, crit } = calcDamage(attacker, target, m, attackerItems, defenderItems);
  const damage = (overtimeMult || 1) * rawDamage;
  const targetPreHp = target.currentHp;
  target.currentHp = Math.max(0, target.currentHp - damage);

  // Focus Sash: survive a full-HP lethal hit with 1 HP (player only)
  if (target.currentHp === 0 && targetPreHp === target.maxHp && tSide === 'player' && target.heldItem?.id === 'focus_sash') {
    target.currentHp = 1;
  }
  // Adrenaline Orb: +1 ATK/Sp.Atk on a super-effective hit
  if (typeEff >= 2 && attacker.heldItem?.id === 'adrenaline_orb') {
    applyStageChange(attacker, 'atk', 1, side, aIdx, events);
    applyStageChange(attacker, 'special', 1, side, aIdx, events);
  }

  events.push({ type: 'attack', side, attackerIdx: aIdx, attackerName: aName,
    targetSide: tSide, targetIdx: tIdx, targetName: tName,
    moveName: m.name, moveType, damage, typeEff, crit, isSpecial: m.isSpecial,
    attackerHpAfter: attacker.currentHp, targetHpAfter: target.currentHp });

  // Life Orb recoil (player only)
  if (side === 'player' && attacker.heldItem?.id === 'life_orb') {
    const recoil = Math.max(1, Math.floor(attacker.maxHp * 0.1));
    attacker.currentHp = Math.max(0, attacker.currentHp - recoil);
    events.push({ type: 'effect', side, idx: aIdx, name: aName, hpChange: -recoil,
      hpAfter: attacker.currentHp, reason: `${aName} lost ${recoil} HP from Life Orb!` });
  }
  // Rocky Helmet
  if (target.heldItem?.id === 'rocky_helmet') {
    const helmet = Math.max(1, Math.floor(attacker.maxHp * 0.12));
    attacker.currentHp = Math.max(0, attacker.currentHp - helmet);
    events.push({ type: 'effect', side, idx: aIdx, name: aName, hpChange: -helmet,
      hpAfter: attacker.currentHp, reason: `Rocky Helmet hurt ${aName} for ${helmet} HP!` });
  }
  // Shell Bell (player only)
  if (side === 'player' && attacker.heldItem?.id === 'shell_bell') {
    const heal = Math.max(1, Math.floor(damage * 0.15));
    const actual = Math.min(heal, attacker.maxHp - attacker.currentHp);
    if (actual > 0) {
      attacker.currentHp += actual;
      events.push({ type: 'effect', side, idx: aIdx, name: aName, hpChange: actual,
        hpAfter: attacker.currentHp, reason: `Shell Bell restored ${actual} HP to ${aName}!` });
    }
  }

  // Faint events (no auto-send — the interactive controller picks the replacement)
  if (target.currentHp <= 0)   events.push({ type: 'faint', side: tSide, idx: tIdx, name: tName });
  if (attacker.currentHp <= 0) events.push({ type: 'faint', side, idx: aIdx, name: aName });
}
